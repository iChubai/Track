...
class ARTrackV2Actor(BaseActor):
    ...
    def forward_pass(self, data):
        ...
        seq_input = torch.cat([torch.ones((B, 1)).to(search_img) * x0, torch.ones((B, 1)).to(search_img) * y0,
                               ...
                               torch.ones((B, 1)).to(search_img) * score,torch.ones((B, 1)).to(search_img) * x0_n,
                               torch.ones((B, 1)).to(search_img) * y0_n, torch.ones((B, 1)).to(search_img) * x1_n,
                               torch.ones((B, 1)).to(search_img) * y1_n, torch.ones((B, 1)).to(search_img) * score_n,
                               ], dim=1)
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        if self.focal == None:
            weight = torch.ones(bins * self.range + 12) * 1
            ...
            weight[bins * self.range + 6] = 0.1
            weight[bins * self.range + 7] = 0.1
            weight[bins * self.range + 8] = 0.1
            weight[bins * self.range + 9] = 0.1
            weight[bins * self.range + 10] = 0.1
            weight.to(pred_feat)

            self.focal = torch.nn.CrossEntropyLoss(weight=weight, size_average=True).to(pred_feat)

        pred = pred_feat.permute(1, 0, 2).reshape(-1, bins * self.range + 12)
