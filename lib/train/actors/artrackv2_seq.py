...
class ARTrackV2SeqActor(BaseActor):
    ...
    def batch_init(self, images, template_bbox, initial_bbox) -> dict:
        ...
        z_2_feat = self.net.module.backbone.patch_embed_origin(z_2_crop)

        out = {'template_images': z_crop, "z_1": z_1_crop, "z_2": z_2_crop, "z_2_feat": z_2_feat}
        return out

    def batch_track(self, img, gt_boxes, template, dz_feat,idx, action_mode='max') -> dict:
        ...
        if idx == 1:        
            outputs = self.net(template, dz_feat.cuda(), x_crop, seq_input=pre_seq_output, head_type=None,
                            ...
                            gt_bboxes=gt_boxes[-1],flag = True)
        else:
            outputs = self.net(template, dz_feat.cuda(), x_crop, seq_input=pre_seq_output, head_type=None,
                               stage="batch_track",
                               search_feature=self.x_feat_rem, target_in_search_img=target_in_search,
                               gt_bboxes=gt_boxes[-1],flag = False)
    ...
    def explore(self, data):
        ...
        for idx in range(np.max(num_frames)):
            ...
            if idx == 0:
                ...
                outputs = self.batch_track(here_images, here_gt_bbox, self.template_temp, self.dz_feat_update,idx,
                                           action_mode='half')
                if outputs == None:
                    return None
    ...
    def compute_sequence_losses(self, data):
        ...
        if self.focal == None:
            weight = torch.ones(self.bins * self.range + 12) * 1
            ...
            weight[self.bins * self.range + 6] = 0.1
            weight[self.bins * self.range + 7] = 0.1
            weight[self.bins * self.range + 8] = 0.1
            weight[self.bins * self.range + 9] = 0.1
            weight[self.bins * self.range + 10] = 0.1
            weight.to(pred_feat)
        ...
        pred = pred_feat.permute(1, 0, 2).reshape(-1, self.bins * self.range + 12)
