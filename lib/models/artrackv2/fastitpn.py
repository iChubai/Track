...
class Fast_iTPN(nn.Module):
    ...
    def forward_features(self,z_0,z_1,x,identity,seqs_input):
        ...
        x_out = self.norm(x[:, -11:-7])
        score_feat = x[:, -7]
        x_n_out = self.norm(x[:, -5:-1])
        score_n_feat = x[:, -1  ]
    ...
    def get_predict_state(self,output_dict):
        return output_dict['feat_n'] , output_dict['score_n_feat']
    ...
    def forward_track(self,z_0,z_1,x,identity):
        ...
        seq = torch.cat([torch.ones((B_x, 1)).to(x) * x0, torch.ones((B_x, 1)).to(x) * y0,
                       ...
                       torch.ones((B_x, 1)).to(x) * score,
                       torch.ones((B_x, 1)).to(x) * x0_n, torch.ones((B_x, 1)).to(x) * y0_n,
                       torch.ones((B_x, 1)).to(x) * x1_n,torch.ones((B_x, 1)).to(x)*y1_n,torch.ones((B_x, 1)).to(x) * score_n], dim=1)
        ...
        output = {'seqs': seqs_output, 'class': values, 'feat': temp, 'feat_n': temp_n, "state": "val/test",
                  "x_feat": output_x_feat.detach(), "score_feat": score_feat,"score_n_feat":score_n_feat}
