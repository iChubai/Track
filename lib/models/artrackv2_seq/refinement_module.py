...
class TokenRefiner(nn.Module):  
    ...
    def refine_next_token(self, next_token, curr_refined, visual_feat, template_feat):  
        """优化下一帧预测token  
        Args:  
            next_token: [4,B,D_token] 下一帧预测token  
            curr_refined: [4,B,D_token] 当前帧优化后的token  
            visual_feat: [B,M,D_feat//2] 搜索区域特征  
            template_feat: [B,M,D_feat//2] 模板特征  
        Returns:  
            refined_next: [4,B,D_token] 优化后的下一帧预测  
        """  
        # 调整形状  
        ...
        # 2. 考虑当前帧优化结果  
        curr_motion = curr_refined - next_token  # [B,4,D_token]  
        motion_feat = self.motion_encoder(curr_motion)  # [B,4,D_feat//4]  
        next_feat = next_feat + motion_feat  
        
        # 3. 视觉引导优化  
        feat_sim = torch.bmm(  
            next_feat,  
            visual_feat.transpose(-2,-1)  
        ) / math.sqrt(next_feat.shape[-1])  
        
        attn = F.softmax(feat_sim, dim=-1)  
        context_feat = torch.bmm(attn, visual_feat)  
        
        # 4. 特征融合与优化  
        fusion_feat = torch.cat([next_feat, context_feat], dim=-1)  
        delta = self.token_refiner(fusion_feat)  
        
        refined = next_token + delta  
