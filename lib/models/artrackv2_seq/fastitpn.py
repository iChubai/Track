...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        self.word_embeddings = nn.Embedding(self.bins*self.range + 12,embed_dim,padding_idx=self.bins*self.range+4,max_norm=None,norm_type = 2.0)
        self.subln = subln
        self.swiglu = swiglu
    ...
    #     xz = self.pos_drop(xz)
    #     return xz
    def forward_track(self,z_0,z_1,x,identity,seqs_input):
        ...
        command= torch.cat([torch.ones(B_x,1).to(x)*x0,torch.ones(B_x,1).to(x)*y0,
                            torch.ones(B_x,1).to(x)*x1,torch.ones(B_x,1).to(x)*y1,
                            torch.ones(B_x,1).to(x)*score,torch.ones(B_x,1).to(x)*x0_n,
                            torch.ones(B_x,1).to(x)*y0_n,torch.ones(B_x,1).to(x)*x1_n,
                            torch.ones(B_x,1).to(x)*y1_n,torch.ones(B_x,1).to(x)*score_n]
                            ,dim = 1)
        ...
        possibility_n = torch.matmul(x_n_out, share_weight)
        out_n = possibility_n + self.output_bias
        temp_n = out_n.transpose(0, 1)

        out_n_list.append(out_n.unsqueeze(0))
        out_n = out_n.softmax(-1)
        ...
        value_n, extra_seq_n = out_n.topk(dim=-1, k=1)[0], out_n.topk(dim=-1, k=1)[1]
        for i in range(4):
            value_n, extra_seq_n = out_n[:, i, :].topk(dim=-1, k=1)[0], out_n[:, i, :].topk(dim=-1, k=1)[1]
            if i == 0:
                seqs_output_n = extra_seq_n
                values_n = value_n
            else:
                seqs_output_n = torch.cat([seqs_output_n, extra_seq_n], dim=-1)
                values_n = torch.cat([values_n, value_n], dim=-1)

        output = {'seqs': seqs_output, 'class': values, 'feat': temp, "state": "val/test", "x_feat": output_x_feat.detach(), "seq_feat": seq_feat,
                  'seq_n':seqs_output_n, 'class_n':values_n, 'feat_n':temp_n,"seq_n_feat":seq_n_feat}

        return output, z_0_feat, z_1_feat, x_feat, score_feat,score_n_feat
    def forward_features(self,z_0,z_1,x,identity,seqs_input):
        B_x,C_x,H_x,W_x = x.shape
        B_z,C_z,H_z,W_z = z_0.shape
        x = self.patch_embed(x)
        z_0 = self.patch_embed(z_0) 
        z_1 =self.downsample_last_dim(z_1,self.embed_dim) 
        # forward stage1&2
        if not self.convmlp and self.stop_grad_conv1:
            x = x.detach() * 0.9 + x * 0.1
        for blk in self.blocks[:-self.num_main_blocks]:
    ...
    def downsample_last_dim(self,x, target_dim, method="softmax"):
        B, N, D = x.shape
        factor = D // target_dim
        if method == "mean":
            x_reshaped = x.view(B, N, factor, target_dim)
            return x_reshaped.mean(dim=2)
        
        elif method == "sum":
            x_reshaped = x.view(B, N, factor, target_dim)
            return x_reshaped.sum(dim=2)
        
        elif method == "max":
            x_reshaped = x.view(B, N, factor, target_dim)
            x_downsampled, _ = x_reshaped.max(dim=2)
            return x_downsampled
        
        elif method == "softmax":
            x_reshaped = x.view(B, N, factor, target_dim)
            weights = torch.softmax(x_reshaped, dim=2)
            return (x_reshaped * weights).sum(dim=2)
        
        elif method == "conv":
            x_in = x.view(B * N, 1, D)
            weight = torch.ones(1, 1, factor, device=x.device) / factor
            x_out = F.conv1d(x_in, weight, stride=factor)
            return x_out.view(B, N, target_dim)
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        if new_patch_size != self.patch_size:
            ...
            old_patch_embed = {}
            for name, param in self.patch_embed_true.named_parameters():
                if 'weight' in name:
                    param = nn.functional.interpolate(param, size=(new_patch_size, new_patch_size),
                                                      mode='bicubic', align_corners=False)
                    param = nn.Parameter(param)
                old_patch_embed[name] = param
            self.patch_embed_true = PatchEmbed_true(img_size=self.img_size, patch_size=new_patch_size, in_chans=3,
                                          embed_dim=self.embed_dim)
            self.patch_embed_true.proj.bias = old_patch_embed['proj.bias']
            self.patch_embed_true.proj.weight = old_patch_embed['proj.weight']
    ...
    def forward(self, z_0, z_1_feat, x, identity, seqs_input, flag,**kwargs):
        """
